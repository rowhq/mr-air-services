...
async function getPreviewConfig(previewId: string): Promise<Partial<FinancingPageConfig> | null> {
  try {
    ...
    return data.config as Partial<FinancingPageConfig>;
  } catch {
    return null;
  }
}

export default async function FinancingPreviewPage({ searchParams }: PreviewPageProps) {
  const params = await searchParams;
  const previewId = params.preview;

  let config: FinancingPageConfig = defaultFinancingConfig;

  if (previewId) {
    const previewConfig = await getPreviewConfig(previewId);
    if (previewConfig) {
      config = deepMerge(defaultFinancingConfig, previewConfig);
    }
  }

  return <FinancingContent config={config} isPreview={true} />;
}
