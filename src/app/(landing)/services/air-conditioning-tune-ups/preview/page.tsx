...
async function getPreviewConfig(previewId: string): Promise<Partial<TuneUpsPageConfig> | null> {
  try {
    ...
    return data.config as Partial<TuneUpsPageConfig>;
  } catch {
    return null;
  }
}

export default async function TuneUpsPreviewPage({ searchParams }: PreviewPageProps) {
  const params = await searchParams;
  const previewId = params.preview;

  let config: TuneUpsPageConfig = defaultTuneUpsConfig;

  if (previewId) {
    const previewConfig = await getPreviewConfig(previewId);
    if (previewConfig) {
      config = deepMerge(defaultTuneUpsConfig, previewConfig);
    }
  }

  return <TuneUpsContent config={config} isPreview={true} />;
}
