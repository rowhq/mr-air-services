...
export interface TuneUpsPageConfig {
  [key: string]: unknown;
  hero: {
    title: string;
    description: string;
...
export const defaultTuneUpsConfig: TuneUpsPageConfig = {
  hero: {
...
interface TuneUpsContentProps {
  config: TuneUpsPageConfig;
  isPreview?: boolean;
}

export function TuneUpsContent({ config, isPreview = false }: TuneUpsContentProps) {
  ...
  return (
    <>
      ...
      {/* Sticky Mobile CTA */}
      {!isPreview && <StickyTuneUpCTA />}

      {/* Hero - FREE Prominently Featured */}
      <section className={`relative min-h-[500px] md:min-h-[600px] lg:min-h-[670px] pt-32 overflow-hidden bg-neutral-900 ${isPreview ? 'mt-8' : ''}`}>
